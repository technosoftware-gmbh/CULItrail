...
describe('resolveBadgeValues', () => {
  ...
  it('reads through the same alias chain the rest of the view uses', () => {
    // Without this a badge configured for reheatTime shows nothing on a note
    // saying `cook:`, while the meta banner beside it shows the value fine.
    // Two parts of one header disagreeing is worse than either being wrong.
    expect(resolveBadgeValues(badge({ property: 'reheatTime' }), { cook: 15 }, settings)).toEqual([
      '15',
    ]);
  });
  ...
  it('formats a minutes badge', () => {
    expect(
      resolveBadgeValues(
        badge({ property: 'reheatTime', valueType: 'minutes' }),
        { reheatTime: 75 },
        settings
      )
    ).toEqual(['1 h 15 min']);
...
describe('badgeLabel', () => {
  it('translates a built-in through its key', () => {
    expect(badgeLabel(badge({ labelKey: 'badges.builtin.cook', builtin: true }))).toBe('Reheat');
  });
  ...
  it('lets an explicit label override the key, which is how editing a built-in works', () => {
    ...
    expect(badgeLabel(badge({ labelKey: 'badges.builtin.cook', label: 'Aufwärmen' }))).toBe(
      'Aufwärmen'
    );
  });
...
describe('planBadgeRow', () => {
  it('plans the shipped built-ins against a full meal', () => {
    // Three built-ins rather than five. Preparation and total went with
    // cooking: a dish that arrives ready has nothing to prepare, and a total
    // over one component only ever restated it.
    const planned = planBadgeRow(
      { diet: 'vegetarian', reheatTime: 30, lastEaten: '2026-07-28' },
      settings
    );
    expect(planned.map((entry) => entry.label)).toEqual(['Diet', 'Reheat', 'Last eaten']);
    expect(planned.find((entry) => entry.label === 'Reheat')?.values).toEqual(['30 min']);
  });

...
describe('the locale the plugin follows', () => {
  ...
  it('offers no override of its own, which is the deliberate part', () => {
    // APERtrail and NODAtrail carry a `language` setting where `auto` means
    // follow Obsidian. CULItrail does not, and its I18nManager header says
    // why: it never disagrees with the app around it, so `initialize()` takes
    // no argument at all. Asserted here rather than left implicit, because a
    // test copied from a sibling would otherwise pass a value that is silently
    // ignored and look like it had checked something.
    expect(Object.keys(DEFAULT_SETTINGS)).not.toContain('language');
  });
});

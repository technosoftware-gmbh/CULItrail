...
describe('readMealMeta', () => {
  it('reads through the configured property names', () => {
    const meta = readMealMeta({ servings: 2, reheatTime: 30 }, settings);
    expect(meta.servings).toBe(2);
    expect(meta.reheatTime).toBe(30);
  });

  it('reads the temperature as written rather than as a number', () => {
    // `95 °C` and `800 W` are both whole answers and neither is a quantity
    // this reader has any business taking apart: one is a temperature and the
    // other is a power, and the only thing done with either is substituting it
    // into a supplier's `{temp}`.
    expect(readMealMeta({ reheatTemp: '95 °C' }, settings).reheatTemp).toBe('95 °C');
    expect(readMealMeta({ reheatTemp: '800 W' }, settings).reheatTemp).toBe('800 W');
  });

  it('reads the appliance as the id a note stores, not a label', () => {
    expect(readMealMeta({ reheatAppliance: 'steamer' }, settings).reheatAppliance).toBe('steamer');
  });
  ...
  it('falls back to an alias when the configured name is absent', () => {
    ...
    expect(readMealMeta({ cook: 15 }, settings).reheatTime).toBe(15);
  });
...
describe('a meal that states no times at all', () => {
  it('reads every reheating field as null rather than as zero', () => {
    // The common case by a wide margin: of 133 notes in the vault this was
    // built against, 119 stated no time whatsoever. Zero would put a "0 min"
    // figure on every one of them.
    const meta = readMealMeta({}, settings);
    expect(meta.reheatTime).toBeNull();
    expect(meta.reheatTemp).toBeNull();
    expect(meta.reheatAppliance).toBeNull();
  });
});

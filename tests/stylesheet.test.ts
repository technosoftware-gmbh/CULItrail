...
/**
 * The core's shipped code, scanned alongside this plugin's own.
 *
 * The invoice renderer moved into the core once a second document needed it,
 * and it still writes `culi-` class names because this stylesheet is the one
 * that ships them. A scanner that looked only at `src/` would report every one
 * of them as an orphan and invite somebody to delete the rules out from under
 * it.
 *
 * `node_modules` rather than a sibling `packages/core/src`. That path was right
 * while the two lived in one repository and the point was to scan the file
 * being edited; the core is a published dependency now and there is no file
 * here to edit. So this reads what is actually installed, which is also what
 * the built plugin will run against.
 *
 * The class names survive compilation as string literals, so scanning `dist`
 * finds the same set the sources would.
 */
const CORE_SRC = join(ROOT, 'node_modules', '@technosoftware', 'trail-core', 'dist');

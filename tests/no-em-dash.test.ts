...
// TypeScript 5, and the version matters.
//
// This sweep asks the TypeScript scanner for comment trivia rather than
// guessing at it with a regex, so it needs the compiler API. TypeScript 7
// moved that off the package root -- `exports["."]` is `lib/version.cjs`, the
// version and nothing else -- and put the scanner behind
// `typescript/unstable/ast` with a changed signature.
//
// This plugin is built with TypeScript 5 for an unrelated reason, written down
// in CONTRIBUTING.md: typescript-eslint 8 refuses to run against 7 at all. If
// that reason ever goes away and the build moves to 7, this import moves with
// it, and the sweep goes quiet rather than loud if nobody notices.
 ...
 */
const SKIP_DIRS = new Set(['node_modules', 'dist', '.git', 'vendor', 'claude-project-bundle']);
 ...
 *
 * One, and it is the repository root. This was a `packages/*` walk when
 * CULItrail lived in the suite, and the shape of the sweeps below is kept so
 * that the two copies of this file still read alike.
 */
...
function topLevelDocs(dir: string): string[] {
  ...
  return readdirSync(dir, { withFileTypes: true })
    .filter((entry) => entry.isFile() && extname(entry.name) === '.md' && entry.name !== RULE_FILE)
 ...
 */
function tokenFindings(path: string, wanted: (kind: ts.SyntaxKind) => boolean): Finding[] {
  ...
  for (let kind = scanner.scan(); kind !== ts.SyntaxKind.EndOfFileToken; kind = scanner.scan()) {
    ...
    findings.push(finding(path, text, scanner.getTokenStart() + token.indexOf(EM_DASH)));
  }
...
function markdownFindings(path: string): Finding[] {
  ...
  const findings: Finding[] = [];
  for (let at = prose.indexOf(EM_DASH); at !== -1; at = prose.indexOf(EM_DASH, at + 1)) {
...
const MARKDOWN_FILES = packageDirs().flatMap((dir) => [
...
describe('the em-dash rule', () => {
  it('reads a meaningful number of files in every sweep', () => {
    ...
    // Floors for one package rather than four. Set below what is actually
    // there so ordinary work does not trip them, and far enough above zero
    // that a walk which stopped finding files cannot pass.
    expect(TS_FILES.length).toBeGreaterThan(250);
    expect(TRANSLATION_FILES.length).toBeGreaterThanOrEqual(2);
    expect(CSS_FILES.length).toBeGreaterThanOrEqual(1);
    expect(MARKDOWN_FILES.length).toBeGreaterThan(20);
  });

  it('is kept in every TypeScript comment', () => {
    expect(report(TS_FILES.flatMap((path) => tokenFindings(path, isComment)))).toEqual([]);
  });
  ...
  it('is kept in every user-facing string', () => {
    ...
    expect(report(TRANSLATION_FILES.flatMap((path) => tokenFindings(path, isString)))).toEqual([]);
  });
  ...
  it('leaves the file that states the rule alone, and that file still states it', () => {
    ...
    expect(ruleFiles.some((path) => readFileSync(path, 'utf8').includes(EM_DASH))).toBe(true);
  });
});

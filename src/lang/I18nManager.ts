...
export class I18nManager {
   ...
   *
   * `getLanguage()` first, and it is the only one of these that reports what
   * the vault owner told **Obsidian** they read. Everything after it reports
   * what they told their operating system, and the two disagree constantly: a
   * Swiss Mac running Obsidian in English answers `de-CH` to moment and to
   * `navigator.language`. This comment promised to follow Obsidian for a long
   * time while the code followed macOS.
   *
   * It mattered more than a mistranslated label, because the folder defaults
   * are localized: a vault set up in English on a German-configured machine
   * had German folder names invented for it beside the English ones it already
   * had. Folder names are written into somebody's vault and nothing renames
   * them afterwards.
   *
   * Available since Obsidian 1.8.7 against a manifest floor of 1.12.0, so it is
   * always there. Guarded anyway: the cost is one `typeof` and the failure it
   * prevents is a crash on load.
   */
  ...
  private getObsidianLanguage(): string | null {
    try {
      return typeof getLanguage === 'function' ? getLanguage() || null : null;
    } catch {
      return null;
    }
  }

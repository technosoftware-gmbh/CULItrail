...
export function mealMetaAliases(settings: CULItrailSettings): MealMetaAliases {
  return {
    ...
    // No alias list beyond the configured name and the default. `temp` alone
    // would collide with the `[temp:: ...]` inline field's name, and there is
    // no older property to find: this one has never been written under
    // another spelling.
    reheatTemp: unique([settings.reheatTempProperty, 'reheatTemp']),
    reheatAppliance: unique([settings.reheatApplianceProperty, 'reheatAppliance']),
...
export function readMealMeta(
  ...
  return {
    ...
    reheatTemp: readString(findValue(frontmatter, ...aliases.reheatTemp)),
    reheatAppliance: readString(findValue(frontmatter, ...aliases.reheatAppliance)),

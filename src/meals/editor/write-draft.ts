 ...
 * the configured one. A vault carrying `cook:` where the setting says
 * `reheatTime` would otherwise come back with both, one of them orphaned. That
...
/**
 * The same, for a value a note states in words.
 *
 * An empty string is written as null so `processFrontMatter` drops the key: a
 * cleared field means the note no longer says how hot, not that it says
...
function writeText(
  ...
  frontmatter[existingKey(frontmatter, aliases, fallback)] = trimmed === '' ? null : trimmed;
}
...
function writeFrontmatter(
  ...
  // Through `writeText` rather than assigned, for the same reason `price` goes
  // through `writeNumber`: clearing the field should take the property off the
  // note rather than leave `reheatTemp: null` on every meal nobody has measured.
  writeText(frontmatter, aliases.reheatTemp, settings.reheatTempProperty, draft.reheatTemp);
  writeText(
    frontmatter,
    aliases.reheatAppliance,
    settings.reheatApplianceProperty,

...
export class EditMealModal extends BaseModal {
  ...
  /**
   * The configured appliances, plus whatever this meal already names.
   *
   * Same rule as the supplier dropdown and the same hazard behind it: a
   * `<select>` whose value matches no option falls back to its first, so a note
   * naming an appliance somebody has since renamed or removed from the list
   * would have that answer quietly replaced by the next save. The unmatched
   * value is offered instead, marked so it does not read as one of the vault's
   * own appliances.
   *
   * The empty option is first and means the meal names none, which is the
   * common case and not an error: a dish nobody has measured yet still has a
   * price, a supplier and a picture.
   */
  private applianceOptions(current: string | null): { value: string; label: string }[] {
    const appliances = this.settings.reheatAppliances;
    const options = [
      { value: '', label: t('meals.editor.noAppliance') },
      ...appliances.map((appliance) => ({ value: appliance.id, label: appliance.label })),
    ];

    const stated = current?.trim() ?? '';
    if (stated !== '' && !appliances.some((appliance) => appliance.id === stated)) {
      options.push({ value: stated, label: t('meals.editor.unknownAppliance', { name: stated }) });
    }
    return options;
  }
  ...
  private renderFields(body: HTMLElement, draft: MealDraft): void {
    ...
    numberFieldRow(basics, [
      {
        label: t('meals.editor.reheatTime'),
        placeholder: '25',
    ...
    // How hot is a text field rather than a number with a unit beside it,
    // because the three appliances do not answer in the same quantity: an oven
    // and a steamer state degrees, a microwave states watts. The placeholder
    // carries the unit so nobody has to guess whether to type one.
    textField(
      basics,
      t('meals.editor.reheatTemp'),
      draft.reheatTemp ?? '',
      (value) => (draft.reheatTemp = value.trim() || null)
    );

    // The options are the vault's own appliance list, not a fixed three, so a
    // household that renamed them or added one gets what it configured. Values
    // are ids and labels are what the list says, which is what keeps a note
    // written in a German vault readable in an English one.
    selectField(
      basics,
      t('meals.editor.reheatAppliance'),
      this.applianceOptions(draft.reheatAppliance),
      draft.reheatAppliance ?? '',
      (value) => (draft.reheatAppliance = value || null)
    );

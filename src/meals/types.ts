...
export interface MealMeta {
  ...
  /**
   * How hot, verbatim as the note states it: `95 °C`, `800 W`. Free text
   * because those two are not the same kind of quantity, and because this is
   * what fills a supplier's `{temp}` token, which has always been a string.
   */
  reheatTemp: string | null;
  /** An appliance id from `reheatAppliances`, or whatever the note says if it names none. */
  reheatAppliance: string | null;
  diet: string[];
  allergens: string[];
  tags: string[];
  favorite: boolean;

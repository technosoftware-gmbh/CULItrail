...
export interface CULItrailSettings {
  ...
  /**
   * How hot, as the packet states it. Free text rather than a number because a
   * microwave states watts and an oven states degrees, and one numeric field
   * with a vault-wide unit could express only one of them.
   */
  reheatTempProperty: string;
  /** Which appliance, as an appliance id from `reheatAppliances`, never a label. */
  reheatApplianceProperty: string;
  dietProperty: string;

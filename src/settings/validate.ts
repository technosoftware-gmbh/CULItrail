...
/**
 * Drops a saved badge that claims to be a built-in this version does not ship.
 *
 * The mirror of `withMissingBuiltins` below, and it exists because removing a
 * built-in went wrong in exactly the way adding one does. A saved list wins
 * outright over the defaults, so a built-in deleted from `defaults.ts` keeps
 * rendering in every vault that has ever opened the badge editor. Its label
 * cannot: the translation tables lost the key with it, so `badgeLabel()` has
 * nothing to resolve and the header prints `badges.builtin.total` at somebody.
 * That is how the Prep and Total badges behaved after the reheating properties
 * replaced them.
 *
 * **Keyed on `builtin: true`, which is what makes this safe.** A badge carrying
 * that flag was put there by this plugin, not typed by a person, so a flag
 * naming a built-in nothing ships any more is unambiguously ours to withdraw.
 * A badge somebody made themselves is never touched, whatever it is configured
 * against, including one they built over the same retired property: that is
 * their arrangement and it still renders under their own label.
 *
 * Not a general "does this property still exist" check, deliberately. A badge
 * against a property no note happens to carry is a badge that renders nothing
 * today and everything tomorrow, and dropping it would delete a row somebody
 * added in advance of the data.
 */
function withoutRetiredBuiltins(saved: CustomBadge[]): CustomBadge[] {
  const shipped = new Set(DEFAULT_SETTINGS.headerBadges.filter((b) => b.builtin).map(builtinKey));
  return saved.filter((badge) => !badge.builtin || shipped.has(builtinKey(badge)));
}
...
function badges(val: unknown): CustomBadge[] {
  ...
  // Retired built-ins go before missing ones are added, so a key that is both
  // gone from the defaults and absent from the saved list cannot be dropped
  // here and re-appended there.
  const live = withoutRetiredBuiltins(kept);
  ...
  return live.length === 0 ? live : withMissingBuiltins(live);
}
...
export function mergeSettings(raw: unknown): CULItrailSettings {
  ...
  return {
    ...
    reheatTempProperty: str(r.reheatTempProperty, d.reheatTempProperty),
    reheatApplianceProperty: str(r.reheatApplianceProperty, d.reheatApplianceProperty),

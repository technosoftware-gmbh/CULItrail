...
export default [
  // Files ESLint should never look at.
  {
    ignores: [
      'node_modules/**',
      // Repository configuration rather than plugin source. It became visible
      // to this config when the plugin moved to its own repository and this
      // file arrived at the root: `.github/cla/signatures.json` is data the
      // CLA bot writes, and eslint parses a stray JSON file as JavaScript.
      '.github/**',
  ...
  // the plugin's real readers rather than reimplementing them.
  {
    ...
    rules: {
      ...
      // `Vault#configDir` is the right way to find the configuration folder
      // from inside the app, and there is no `Vault` out here: a script has a
      // path and nothing else. The one script that reads a vault's own
      // `data.json` defaults to `.obsidian`, prints the folder it used and
      // takes `--config-dir` for a vault that renamed it, which is as close to
      // asking as a terminal gets.

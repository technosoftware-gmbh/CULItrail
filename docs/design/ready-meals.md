...
**Ordinarily, three frontmatter properties**, because a dish that is bought
ready-made is reheated one way:

```yaml
reheatAppliance: steamer
reheatTemp: 95 °C
reheatTime: 25
```

`reheatTemp` is **free text, not a number**, and that is the part worth not
undoing. A steamer and an oven state degrees; a microwave states watts. One
numeric field with a vault-wide unit could express only one of the two, and a
second field for the unit would be a second thing a hand edit can contradict.
It is also what fills a supplier's `{temp}`, which has always substituted a
string. `reheatAppliance` holds an **id**, never a label, so a note written in a
German vault still resolves in an English one.

The appliance is required and the two figures are not. Without one there is no
heading to resolve against, so there is no supplier wording to fill and no label
to render; picking the supplier's only appliance would be the plugin answering a
question the note declined to answer.

**This replaced the counting that follows.** The section below is still legal
and still wins, but it described what one note in the library actually did. Of
133 meal notes, **one** carried a reheating section, and **none** resolved an
instruction at all: every dish came from a supplier whose wording carries both
tokens, and nothing on any dish filled them. A note format that the whole
library declines to use is not a format anybody chose against; it is one nobody
could fill in. Three properties in the editor's own form are what the library
can state.

### The meal note, spelled out

A section under the configured reheating heading, with one sub-heading per
appliance, **overriding the properties for the appliance it names**. Per
appliance rather than all-or-nothing: a rule reading "frontmatter unless a
section exists" would make writing one appliance by hand silently delete the one
in the properties.
...
so it is stated as a table. "Dish says" is the frontmatter and the body section
together, the section winning where both name an appliance, so the table below
did not change when the properties arrived: frontmatter states values and no
prose, which is a row it already had.

For one dish and one appliance:

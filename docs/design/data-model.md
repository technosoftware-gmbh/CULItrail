...
- A meal's **reheating instructions**, merged per appliance from what the note
  ...
  recording the result: a note that names an appliance and a temperature, or
  grows a `## Steamer` sub-heading, becomes reheatable by doing so. See
  [Ready meals](ready-meals.md).

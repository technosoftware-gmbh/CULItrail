...
## How a meal states its reheating

**Three frontmatter properties, and the body section is the override.**
`reheatAppliance` (an appliance **id**, never a label), `reheatTemp` (**free
text**) and `reheatTime` (minutes). `from-frontmatter.ts` turns them into one
`ApplianceEntry` carrying values and no prose, which is a row the merge table
already had, so frontmatter joined the merge without the merge changing. A body
section still wins, **per appliance rather than all-or-nothing**: the other rule
would make writing one appliance by hand silently delete the one in the
properties.

**`prepTime` and `totalTime` are gone, and the counting is why.** A meal is
bought ready-made, so preparation describes nothing anybody can act on, and with
preparation always absent a total could only ever restate the reheat time: on
screen it was two adjacent columns of one strip holding the same number, which
is the two-names-for-one-value condition `default_serving_size` was deleted for.
Counted in the vault this was built against: **133 meal notes, all 133 carrying
both keys, 10 stating a preparation time and 13 a total**, and the ones stating
both disagreed with their own arithmetic (35 + 35 recorded as a total of 35).
`scripts/strip-meal-times.ts` takes them off, and **reports before it deletes**:
a plain `--apply` removes only the blank keys and lists every note that states
something, which `--apply --discard-values` then removes. Unlike the two strip
scripts beside it the values are not dead by construction, so silent deletion
would be discarding something somebody typed.

**`reheatTemp` is a string on purpose.** A steamer and an oven state degrees, a
microwave states watts. One numeric field with a vault-wide unit could express
only one of them, a second field for the unit is a second thing a hand edit can
contradict, and `{temp}` has always substituted a string anyway.

**The shipped Prep and Total badges went with them**, and `timeBadgeKind()` has
one kind instead of three. **Removing a built-in takes two edits, and the first
release of this shipped only one.** A saved `headerBadges` list wins outright
over the defaults, so deleting a badge from `defaults.ts` removes it from a
fresh vault and from nowhere else. What made that visible rather than merely
stale is that the label went with it: `badgeLabel()` had nothing to resolve and
`t()` answers an unknown key with the key, so a real header printed
`BADGES.BUILTIN.TOTAL` over a figure its formula still computed from a property
the migration had just deleted.

`withoutRetiredBuiltins()` in `settings/validate.ts` is the second edit, and the
mirror of `withMissingBuiltins()` beside it: between them they are the whole
lifecycle of a built-in, which cannot be managed from `defaults.ts` once a vault
has saved a list. **It keys on `builtin: true`**, which is what makes deleting a
row from somebody's header defensible: that flag is the plugin's own, so a badge
carrying it and naming a built-in nothing ships is ours to withdraw, while a
badge somebody made themselves stays exactly where it is even when it names the
same retired property. It is deliberately not a check for whether a property
still exists: a badge over a property no note happens to carry renders nothing
today and everything tomorrow.

**Two things this had to reach across repositories for.** `MealDraft` is
`trail-core`'s, not this plugin's, so dropping two fields and adding two is a
core change and a core release before CULItrail can build against it. Nothing
else needed one: `ApplianceEntry` already carried `temp` and `time` as strings,
which is the only reason the merge itself was untouched.
...
- **Some fields are derived, never written back.** A meal's reheating
  instructions, merged per appliance from the dish and its supplier, are the
  standing example; a meal's supplier, taken from the newest order naming the
  dish, is the other. An explicit value in the note always wins over the derived
  one. Writing the derived value back would mean editing one note as a side
  effect of editing another, and it would go stale the moment its source
